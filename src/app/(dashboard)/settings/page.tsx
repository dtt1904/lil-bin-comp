...
const INTEGRATION_ICONS: Record<string, typeof Globe> = {
  [IntegrationType.GOOGLE_DRIVE]: HardDrive,
  [IntegrationType.GOOGLE_SHEETS]: FileSpreadsheet,
  [IntegrationType.META_FACEBOOK]: Globe,
  [IntegrationType.STRIPE]: CreditCard,
  [IntegrationType.WEBHOOK]: Webhook,
  [IntegrationType.CUSTOM]: Puzzle,
};

const INTEGRATION_STATUS_COLORS: Record<string, string> = {
  [IntegrationStatus.ACTIVE]: "bg-emerald-500/15 text-emerald-400",
  [IntegrationStatus.INACTIVE]: "bg-zinc-500/15 text-zinc-400",
  [IntegrationStatus.ERROR]: "bg-red-500/15 text-red-400",
};

const MODULE_STATUS_COLORS: Record<string, string> = {
  [ModuleStatus.ACTIVE]: "bg-emerald-500/15 text-emerald-400",
  [ModuleStatus.INACTIVE]: "bg-zinc-500/15 text-zinc-400",
};
...
function getWorkspaceName(workspaceId?: string) {
  if (!workspaceId) return "Organization-wide";
                ...
                <Card key={integration.id}>
                  <CardContent className="space-y-4">
                    ...
                    <p className="text-xs text-muted-foreground line-clamp-2">
                      {formatConfigSummary(integration.config)}
                    </p>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                          ...
                          {getWorkspaceName(integration.workspaceId)}
                        </span>
            ...
            {moduleInstallations.map((mod) => (
              <Card key={mod.id}>
                <CardContent className="space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatConfigSummary(mod.config)}
                  </p>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Building className="h-3 w-3" />
                        {getWorkspaceName(mod.workspaceId)}
                      </span>
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" />
