...
function getWorkspaceName(workspaceId: string) {
  return workspaces.find((w) => w.id === workspaceId)?.name ?? "Global";
}
...
export default function KnowledgeCenterPage() {
  ...
  const filteredSOPs = useMemo(() => {
    if (!searchQuery) return sopDocuments;
    const q = searchQuery.toLowerCase();
  ...
  const filteredPrompts = useMemo(() => {
    if (!searchQuery) return promptTemplates;
    const q = searchQuery.toLowerCase();
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {memoryEntries.length} memories &middot; {sopDocuments.length} SOPs
            &middot; {promptTemplates.length} prompts
          </p>
        </div>
        <DropdownMenu>
        ...
        <TabsContent value={0} className="mt-6 space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {["ALL", ...Object.values(MemoryType)].map((type) => (
                <button
                  key={type}
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {isExpanded && (
                      <div className="ml-6 space-y-3 border-l-2 border-border/50 pl-4">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Created by {getUserName(memory.createdById)}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="ml-6 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {memory.workspaceId
                          ? getWorkspaceName(memory.workspaceId)
                          : "Global"}
                      </span>
                      {memory.agentId && (
                        <span className="flex items-center gap-1">
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Building className="h-3 w-3" />
                        {sop.workspaceId
                          ? getWorkspaceName(sop.workspaceId)
                          : "Global"}
                      </span>
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" />
                        Updated {formatRelativeTime(sop.updatedAt)}
                      </span>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Building className="h-3 w-3" />
                        {prompt.workspaceId
                          ? getWorkspaceName(prompt.workspaceId)
                          : "Global"}
                      </span>
                      <span className="flex items-center gap-1">
                        <Calendar className="h-3 w-3" />
